<p align="center">
  <img src="https://capsule-render.vercel.app/header?type=waving&color=0D1117&height=200&section=header&text=Lucas%20Tolentino&fontSize=70&animation=fadeIn&fontColor=ffffff" width="100%" />
</p>

<p align="center">
  <img src="https://avatars.githubusercontent.com/u/265619879?v=4" width="150" style="border-radius: 50%; border: 5px solid #0D1117; margin-top: -100px; position: relative;" alt="Lucas Tolentino" />
</p>

<p align="center">
  <strong>🚀 Backend Developer focado em automação de processos e IA</strong><br>
  Brasil
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/lucas-vinicius-3091322ba" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  &nbsp;
  <a href="mailto:lucasvinicius027@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

---

### 🧠 Sobre Mim
Sou estudante de Ciência da Computação (3º período) na Estácio, com foco total em desenvolvimento backend e automação de processos. Atuo como Freelancer em consultoria tecnológica, transformando fluxos manuais em sistemas inteligentes e escaláveis.

### 🛠️ Tecnologias & Ferramentas
| Categoria | Tecnologias |
| :--- | :--- |
| **Backend** | Python, Automação de Processos (BPA) |
| **Dados** | Pandas, ETL, SQL, Scraping |
| **Automação** | n8n, Z-API, Webhooks, Selenium, PyAutoGUI |
| **Ferramentas** | Git, GitHub, VS Code, Linux, Miro |

---

### 🐍 Atividade no GitHub
<p align="center">
  <img src="https://raw.githubusercontent.com/lucas-tolentino/lucas-tolentino/output/github-snake-dark.svg" />
</p>
